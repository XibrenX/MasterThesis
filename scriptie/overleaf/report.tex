...
FOr references we can use two sources. The first is Aminer, which is a dataset
that has the articles, references and authors.
The second is opencitations. Opencitations is a service which is DOI based.
Selection of which source to use is based on two factors:
\paragraph{Integrity}
Aminer has article, author and reference. The DOI of articles is not fully 
filled \todo{Getallen}. FOr articles without a DOI the integrity should be Based
on multiple attrubutes. We can use the title in combination with the authors. 
However is is not uncommon that an article which is written by the same authors
is being republished. In theory, this publication is different.

Opencitations is DOI based. So the cited and citing articles are referred to by
doi.
...
Considering the dataflow in Figure~\ref{fig:dataflow_jm2017}, this is the 
sigma-sign.
...
By scraping the books we also get the link to the front matter pdf. We use this
to download the front matter and process this downloaded PDF by the tool 
discussed before.

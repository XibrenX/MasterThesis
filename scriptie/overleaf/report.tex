...
To answer this question, we will perform a case studies for three groups:
\begin{itemize}
...
And how can we integrate this data to perform an outlier detection.
...
This makes the model far more easy to reason about when person has multiple 
roles.
...
Modelling (Data Vault or Anchor modelling). However the physical model needs to 
...
from this entities, the attributes are not that much. This set was created for
...
\subsection{OpenCitation} is DOI based. So the cited and citing articles are 
referred to by a doi. For references we use opencitations. Because opencitations is DOI based, it is 
very good integrable with LNCS.
...
trying to scrape the site. So the acquisition of data is not pratical.
...
\begin{itemize}
    ...
    \item Other roles than `author' is not available in public datasets.
\end{itemize}
...
% ==============================================================================
\subsection{Articles}
The unique key which is used identity an article is the Digital Object 
Identifier. This field is available in DBLP, Aminer and OpenCitations. DOI 
should be unique across all articles. So, theoretical this should be easy going.
...
For DBLP approximaly 78\% has one or multiple doi's.

Aminer provides one doi field for an article. Also not all DOI's are not available.
From the 4894081 articles in Aminer, 3920939 has a DOI (approximally 80\%).
However, the doi's provided in the dataset are not unique: 3316 DOI's are used
to identity multiple articles. A quick investigation learned us that Aminer 
...
% ==============================================================================
Let's start with the title; the title and how sources save this title may differ. 
...
with specific synbols (e.g. Greek delta-sign) which are expressed differently 
...
The format issue and alias issue is covered by DBLP to create a master 
...
(however, also data we do not need, but we do not know that at forehand) and it 
...
sources. However, this approach leads to a lot of tables, costs storage and 
...
download link to the front matter.
...
involved, so while we are scraping the articles, we can scrape the authors as 
well.
...
important for this case study. For all chapters from we scraped the DOI. 
...
files in to information. Because of the effort and size, we elaborate on this 
ste in Chapter~\ref{chp:front_matter_parsing}.
...
For citations we described two sources in Chapter~\ref{chp:data} may be 
sufficient. The first is Aminer, 
which is a dataset that has the articles, references and authors.
The second is OpenCitations. OpenCitations is a service which is DOI based.

\paragraph{Aminer} has article, author and reference. The DOI of articles is not fully 
filled. For articles without a DOI the integrity should be based
on multiple attributes, e.g. the title in combination with the authors. 
However it is not uncommon that an article which is written by the same authors
is being republished. In theory, this publication is different.

% is verplaatst naar


\paragraph{OpenCitation} is DOI based. So the cited and citing articles are 
referred to by a doi. Because opencitations is DOI based, the integrability depends
on the availability of DOI's.

\todo{Onderbouwing met getallen}
For our case study we used opencitations. We have all doi from chapters from the
LNCS website, so the from is covered.
The dataset we are going to use to get the referred article, is coming from DBLP,
which is quite complete in case of doi.

The dataset from opencitation is a download, but very large. For this reason
we choose to filter the data at loading, so we only load the references from an
article in LNCS.
...
\begin{description}
    ...
    integration of this API source is not needed, we will integrate with the 
    bulk data from DBLP.
\end{description}
...
case~\ref{sec:work_member_editorial_board_cited}, we need the number of issues 
here.

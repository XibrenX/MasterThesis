...
deterrents, the scientific process would be inundated by dishonest behaviour. 
...
Despite these deterrents, dishonest behaviour whose methods are beyond detection 
by these measures, still occurs. Typically, these were discovered by whistle 
blowers and manual investigation. However, manual investigation does not scale. 
In particular, an obstacle is the huge (and growing) number of scientific 
publication and scientific authors. For example, the DBLP website, which records 
most publications in computer science, currently lists more than 2.5 million
authors -- a number that cannot be manually investigated. Hence, a more generic 
approach to detect fraud is needed. 

In this project, we aim to take the first step in this direction by not focusing 
on the method of fraud applied, but on the \emph{effect of scientific fraud}. 
The underlying assumption is that the various types of scientific fraud all 
share a common characteristic: their goal is the same, that is, their goal is to 
improve a researcher's or publication's quantitative measures of science. This 
elevates them above their peers -- which on the one hand brings recognition and 
accolades, but on the other, it makes them stand out. In short, the underlying 
assumption is that fraudsters aim to become outliers (in terms of quantitative 
measures of science).

We immediately caution that this implication does not hold in reverse. That is: 
such outliers need not be fraudsters. Specifically, as the quantitative measures
aim to detect scientific excellence, excellent scientists will also be outliers.
...
%% \begin{description}
    %% \item[Detection of plagiarism:] To detect plagiarism, most publishers use a
        %% service which detects the equality of text with other publications, such as
        %% Ithenticate.
    %% %% Reviewers are encouraged to use these services.
    %% \item[Preventing manipulation of research data:]
        %% The common way to prevent the manipulation of research data is to preregister
        %% your research. Preregistration is sharing of your research design and the
        %% method before executing the actual research. The goal is to ensure
        %% transparency, prevent data manipulation (such as p-hacking), and to execute
        %% the research according to plan. Preregistration is not mandatory. By
        %% preregister a research, the author is certain that the publication can not be
        %% rejected due to its method \cite{GC2015}. 
    %% \item[Detection of manipulated research data:]
        %% Furthermore, there are ongoing efforts into applying statistical evaluation
        %% to uncover manipulation of research data. Research of Hartgerink and Van
        %% Assen is focused on this topic, e.g.~\cite{HGWA2019}.
    % \item [detecting manipulation of research data:]
    %     %%Besides preregistration as countermeasure for research data manipulation,
    %     Statistical evaluation may uncover manipulation of research data, e.g.~\cite{}.
    %%research is conducted in the area of detecting manipulation of research data by
    %%applying statistical methods. 
    %% \item[Detecting manipulated images:] Several tools can detect manipulated images,
        %% such as ORI's Forensic Droplets\footnote{\url{https://ori.hhs.gov/droplets}}.
        %% Furthermore, research how to
        %% detect image manipulation is ongoing, e.g.,~\cite{KWI2017}.
%% \end{description}
%% The fact that there is research conducted in detecting data manipulation and image
%% fraud, does not mean that prevention strategies are structural in place. People
%% engaged in the publication process need to run this tool for every submitted paper.
%% Tooling to detect plagiarism is widely available and making use of these is embedded
%% in the review process. 
%% As mentioned, these prevention strategies has the disadvantage
%% of only detecting one type of fraud. 


%% Detecting other types of fraud (not the regular kind) is not in place;
%% countermeasures only exist for known attacks. Continuing this way results in one
%% specific countermeasure for one type of discovered attack. This is not a sustainable
%% situation, countermeasures are overtaken by events.
...
of the publication process and the incentives of the authors. Our proposal is to use
machine learning techniques to detect authors which are outliers, given the metrics
of the publication process. The goal we aim to achieve with our proposal is to focus
the limited capacity of manual fraud investigation to strongly increase the
probability of identifying the most egregious of fraudsters. 
% ------------------------------------------------------------------------------
...
% is based on the common goal of these attacks, which is to skew metrics in favor of the author. By 
% using machine learning to detect outliers given the metrics of the publication process, we can identify authors
% who need further manual investigation.

% The goal of this project is to use automation to focus the limited capacity of
% manual fraud investigation to strongly increase the probability of identifying
% the most egregious of fraudsters. 


% The main reason is that we do not know
% how the fraud is conducted \HJ{`the fraud' -- je bedoelt `fraud', maar dan moet je het subtieler
% formuleren. We weten namelijk van sommige soorten fraude wel hoe ze gepleegd worden, maar niet
% zomaar van alle. EN DAT IS NU NET DE KRACHT VAN WAAR WE HEEN WILLEN (dans op stoel).}.
% From the ones that are detected, we notice that the trigger \HJ{trigger??}
% is mostly based on gut feeling. Also, figuring out what happened asks a lot of human
% effort.\HJ{Englishify please}


%%     \item In the detection and prevention measurements we miss the possibility to structural detect attacks on the publication process.

% \outline{
% \begin{itemize}
% \item Dus: tegenmaatregelen zijn nu specifiek tegen een specifiek type aanval
% \item Als er dan een nieuw type aanval komt, is daar geen maatregel tegen
% \item Zo krijg je een kat-en-muisspel: voor iedere aanval die we kunnen bedenken,
%     moet er een nieuwe preventiemaatregel komen.
% \item Dat is niet echt efficient, kan het ook beter?
% \item JA!
% \item Namelijk: al deze aanvallen hebben eenzelfde doel: metrieken verbeteren
% \item \HJ{Als het past, zou je dit punt ook kunnen maken:}
%     Extra bonus voordeel: zo kun je ook aanvallen detecteren op delen van het
%     publicatieproces waar geen data voorhanden is (bijv.~Moon-achtige aanvallen op
%     reviewing).
% \item \ldots
% \end{itemize}
% }

% %% This means that these forms of fraud can pass undetected.
% However, all forms of fraud will, in general, have similar effects
% \HJ{ben er niet van overtuigd dat dit zonder meer waar is\ldots verdient wat meer
% argumentatie denk ik}: they impact metrics of the publication process. After all,
% the purpose to commit \HJ{of committing} fraud is to adjust \HJ{skew/tweak?} those
% metrics in favor of the author, given the scientific incentives.
% \HJ{Directe versie is niet noodzakelijk waar: je zou ook kunnen proberen anderen
% te beschadigen. Dat levert je dan indirect voordeel.}
%     % \item However: scientific publication will, in general, have similar effects: they impact
%     %     ``kengetallen''.
% Our idea \HJ{The goal of this project} is to detect structural fraud \HJ{``structural
% fraud''? die term is nog niet geintroduceerd.} on the publication process by
% monitoring these metrics.
% \HJ{``monitoring'': denk het niet. Dat betekent dat je ze constant in de gaten houdt.
% Wij willen een offline analyse kunnen doen, niet eentje die continue kijkt welke
% data er binnenkomt en wat er dan gebeurt. Dacht ik tenminste.}
%     % \item Dus, bright idea: fraude in het publicatieproces kan worden opgemerkt door
%     %     kengetallen van het publicatieproces te monitoren.
    
% The goal we want to achieve is to define a method which filters authors who may be
% involved 
% \HJ{te vaag -- alle auteurs ``may be involved''}
% in consequent \HJ{???} structural attacks \HJ{not quite English} on the
% publication process. Authors who are gaming the publication
% process once or whose gaming does not significantly impact their metrics, are not of interest;; only these authors who
% benefit are
% of interest. After all, if one did commit fraud but did not
% benefit, he or she just failed in gaming the publication process. 


    
%%During our process working towards this goal, we expect to deliver the following contributions:
% We plan the following contributions:
% \HJ{My bad -- ik heb altijd moeite met de balans tussen research questions
% en contributions. (Contributions zijn veel beter, maar RQ's zijn verplicht *sad smiley*).
% Nu ik het zo in context lees, is het duidelijk dat de contributions: (1) pas
% na de RQs genoemd moeten worden; (2) dat (wat mij betreft) het VAF draait om RQs
% voor je scriptie te defini\"eren.  (nou ja, wat mij betreft\ldots als RQs verplicht
% zijn in je scriptie \'en je hebt ook nog een VAF, dan is het logisch dat het VAF
% resulteert in de RQs voor je scriptie.)

% In je scriptie kun je dus prima contributions (van
% het project) noemen; hier zou je alleen contributions van het VAF-verslag moeten
% noemen. Dat zijn typisch: initi\"ele verkenning $+$ RQs $+$ methodologie  $+$ planning.
% Dat hoef je niet per se expliciet te maken. Dus copy-paste onderstaande contributions
% maar alvast aan thesis.tex.}
    
    %% we willen 'wat' kunnen; dat moet hypothese zijn
%%    \item Our assumption is that some fraud on the publication process can be detected by monitoring certain features of the process.
...
\old{
\begin{description}
    \item[A formal model of the publication process] The publication process consists of inputs
        (features) and outputs (metrics). Simply defined; the output is depended on the inputs,
        and those inputs are a result of the behaviour of the author. The output is the metric the author want to change in his (or her) favor. To determine what
        features we need to monitor, we need to understand how this publication process works.
        To achieve this, we need to have a formal model of the process to analyse these relations, which is our second contribution:
    \item[Deriving input-output relations] Given this formal model, we can derive all inputs that affect
        the output metrics, which are the metrics an attacker wants to adjust. This is important because these relations give us insights how the publication model can be played. With the inputs of these relations we can start putting together a list of features we need to collect. We will formalise
        a few metrics and elaborate how to game this model to adjust them in favor of the attacker. Except these theoretical gaming, we also know some attacks on the model which actually did took place, which leads to our third contribution.
    \item[Overview of known attacks] We investigate known attacks and show how to model these
        attacks in the formal model. This substantiates that the formal model is relevant for
        detecting practical attacks. Combining these with the formal attacks brings us to our fourth contribution.
    \item[Features indicating an attack] We combine the knowledge gained from establishing
        input-output relations and the theoretical embedding of practical attacks in the formal
        model to derive features that are indicative of metrics, and therefore, possibly abused in
        attacks. Having this features, we can proceed with the detection method.
    \item[Detection method] With these features in hand we propose a formal approach to automatically
        separate ``interesting'' cases from regular scientists. Consider these features as
        input for our detection mechanism, the output is a set of regular authors and a set of
        authors that are positive outliers on (combinations of) those features.
\end{description}
}
...
Incentives for scientists rely on quantitative measures of science, 
    such as number of publications, number of citations and amount of grants 
    received. 
This gives scientist a higher status, salary, more resources (funds, students) to do 
    more research (which leads to more rewards); recognition and accolades.
To achieve these rewards, scientists need to have their papers 
    published for their career to 
    thrive. 
This process of publishing and rewarding is shown by Bj\"ork and 
    Hedlund~\cite{BH2004} in a cycle within their visualisation of the 
...
In this image we see that the researcher is evaluated and receives 
    incentives (such as funds) based on performance measures. In their paper,
    Bj\"ork and Hedlund mention that for public bodies that provide funds
    (which is an incentive for the researcher), the production and consumption
    of publications should be optimized.
...
\item dat moet dus gebaseerd zijn op een holistische view van het publicatieproces (kerngetallen bijvoorbeeld)
    -- en weten wat daar de norm is.
\item Dus beschouwen we in de volgende secties het publicatieprocess in meer detail.
\end{itemize}
}
...
Two main targets exists for scientific publication: journals 
    (incollection) or conferences (inproceeding). While journal publications 
    are the norm in many other disciplines, conference publications are
    common in Computer Science.
The next sections describe these processes from a Computer Science 
    perspective. For other disciplines these processes may differ.
    For example, in CS conferences, rigorous peer-review is the norm, with
    acceptance rates of 20\% or less being common;
    in other disciplines, peer review may be less strict.
...
First we will discuss the process for publishing in a journal. Although the exact process may differ across journals, there is a certain common process. We tend to describe this common process.

The process of publishing in a journal starts at the author who has a manuscript he wants to have published.
This can be based on a 'call for papers', or on own initiative.
According to Cormode, the Editor-in-Chief receives the manuscript and do a minor check if the paper is good enough for further processing.
If it is, the further process of handling the paper is assigned to an associate editor who also perform some checks for quality (understandability, duplication of prior work, topic in scope). The checks from the editor-in-chief and associate editor can lead to a rejection of the manuscript.
According to Cormode, the author is encouraged to suggest an associate editor at submitting. This helps the editor-in-chief to delegate the handling to a associate-editor with knowledge of the domain e.g..

The main responsibilities of the associate editor are 1) Initial handling and selecting reviews for the papers and 2) obtaining a decision for a paper.
Although selecting reviewers to review a manuscript is a responsibility of the associate editor, it is not uncommon for an author to suggest reviewers. Sometimes he even needs to provide reviewers.

After getting the results from the reviewers, the associate editor decides if the manuscript needs to be adjusted.
Formally the reviewers give an advise, but it is up to the associate editor to do the final judgement.
If adjustments are needed, the author adjusts his manuscript and this needs to be reviewed again. This can happen with other reviewers.
An alternative path is that an author chooses to withdraw his paper.

Eventually, hopefully, if the paper is accepted by the associate editor, an advise will be given to the editor-in-chief.
Formally the editor-in-chief decides if a paper is being published, but most of the time the advise of the associate editor is adopted.
This chain of advise and assignment is shown in Figure~\ref{fig:c2013}. Interesting in this schematic overview is the absolute power of the Editor-in-Chief. He can make a judgement without the advise of the associate-editor (and reviewers). Also this role can influence the process by choosing a specific associate-editor. Under the Editor-in-Chief, we draw the Associate editor. This role has two ways to influence the process; by choosing the reviewers and by giving the advise to the editor-in-chief.
The reviewers have less power; off course they can provide an advise, but most of the cases there are multiple reviewers.
...
% \paragraph{IDEF0}
% \vraag{Misschien als een soort intermezzo?}
% \begin{itemize}
%     \item The models of Björk and Hedlund are based on the IDEF0 specification. This is a modelling type for business processes.
%     \item Every box represents an activity. In Figure~\ref{fig:idef0} an IDEF0 activity is shown. This activity has inputs to the left and outputs to the right. To perform such activity, resources are needed (e.g. people, tools). These resources are drawn from below in the activity. In Figure~\ref{fig:bh2004_a22331} we can see Publisher en Editor as a resource to perform 'Manage the review process'. To execute an activity, sometimes controls are needed such as plans, specification, or in this case, journal review policies.
% \end{itemize}
% \begin{figure}[H]
% \centering
% \includegraphics[width=6cm]{images/idef0.drawio.png}
% \caption{IDEF0}
% \label{fig:idef0}
% \end{figure}
...
% \subsection{IEEE}
% (https://ieeemce.org/planning-basics/getting-started/assembling-your-committee/):
% \paragraph{General Chair}: Appointed by the sponsoring organization. Often responsible for selecting the conference location. Appoints the chairs of other committees that make up the conference committee.

% \paragraph{Finance Chair \& Treasurer}: Depending on the size of the conference, a committee may be necessary to ensure all financial, tax, and audit requirements are met. Some of these requirements include establishing conference bank accounts, indirect tax, insurance and bonding, budget, expenses, financial reports, and conference closing. Also responsible for the final audit, if required.

% \paragraph{Program Chair}: Ensures that a well-balanced, high-quality program is organized and presented at the conference. Handles the Call for Papers through the selection and review of every paper. Assists in the scheduling of session rooms and helping with local arrangements for the program.

% \paragraph{Publications Chair}: Responsible for the coordination of conference content production (e.g., papers from special tutorial sessions or colloquia, summaries of conference papers, programs, etc.) and serves as the point of contact for all Xplore® submission-related inquiries before and after the conference.

% \paragraph{Publicity \& Public Relations Chair}: Gathers, maintains, and utilizes lists of media contacts and past and potential attendees for targeted outreach; promotes the conference through placements in various publication calendars and advertisements in IEEE and non-IEEE publications and news media. May also develop and manage the website and any social-media applications.
% }
% EIND KOPIE

    ...
    public static class DatabaseHelper
    {
        public static void WriteToDb(string connStr, string schemaName, string tableName, DataTable dt)
        {
            ...
            else
            {
                ...
                {
                    // skip id fields.
                    if (dtColumn.Key.Equals("$_object_id") || dtColumn.Key.Equals("$_parent_object_id"))
                    {
                        continue;
                    }
                ...
                if (columnsToAdd.Count > 0)
                {
                    ...
                    {
                        columnAdd.Add($"[{c.Key}] NVARCHAR({c.Value})");
                    }
                ...
                if (columnsToChange.Count > 0)
                {
                    ...
                    {
                        string qry = $"ALTER TABLE [{schemaName}].[{tableName}] ALTER COLUMN [{c.Key}] NVARCHAR({c.Value})";
                        if (c.Key.StartsWith("$_"))
                        {
                            qry = $"ALTER TABLE [{schemaName}].[{tableName}] ALTER COLUMN [{c.Key}] VARCHAR({c.Value}) NOT NULL";
        ...
        private static string GetCreateTableStatement(DataTable dt, string schemaName, string TableName)
        {
            ...
            {
                "[$_valid_on] DATETIME2(0) DEFAULT '2021-04-06 23:00:00' NOT NULL"
            };
            ...
            foreach (DataColumn dc in dt.Columns)
            {
                ...
                if (dc.DataType == typeof(string))
                {
                    ...
                    columnPart = $"[{dc.ColumnName}] {dataType}({stringLength})";
                }
                else if (dc.DataType == typeof(int))
                {
                    columnPart = $"[{dc.ColumnName}] INT";
                }
                else
                    throw new Exception("Dont know what to do.");
                if (dc.ColumnName.StartsWith("$_"))
                    columnPart += " NOT NULL";
                if (dc.ColumnName.Equals("$_parent_object_id"))
                {
                    isNotRoot = true;
                }
            ...
            if (isNotRoot) pk = "[$_parent_object_id], ";
            pk += "[$_object_id]";

            string primaryKey = $", CONSTRAINT PK_{TableName} PRIMARY KEY CLUSTERED({pk})";

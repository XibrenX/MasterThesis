...
with book_members as (
    ...
    LEFT OUTER JOIN {{ ref('stg_lncs_fm_member_book') }} m
...
, book_chapter_author_cte AS (
    ...
    left outer join {{ source('springer_lncs', 'chapter') }} c
    on b."$_book_url" = c."$_book_url"
    left outer join {{ source('springer_lncs', 'author') }} a
...
, member_authoring as (
    ...
    (
        SELECT 
          ...
          , m.member_name
          , bca.author_name
        FROM {{ ref('stg_lncs_fm_member_book') }} m
...
SELECT 
      bm.dblp_key as book_dblp_key
    , bm.p_members as members_in_book
    , ma.member_name

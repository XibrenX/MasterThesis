...
WITH member_role_cte AS (
	SELECT
	    ...
	    , CASE
		    ...
		    ELSE '-'
		END as "clean_role"
	FROM lncs_front_matter_w.member
)
...
, section_validation_cte AS (
    ...
    , CASE 
        ...
        THEN
          CASE 
            ...
            ELSE 'FAIL'
          END
      	WHEN (s.parser = 'OneLastNameFirstNameParser')
			THEN
			  CASE
             WHEN ((s.num_section_lines_non_empty - s.num_merged_lines) = m.num_members) 
			      THEN 'PASS'
    ...
    FROM lncs_front_matter_w.section s
...
, file_section_cte AS (
    ...
    LEFT OUTER JOIN lncs_front_matter_w.section s
    ON f.id = s.file_id
    GROUP BY f.id
)
, file_cte AS (
    ...
    FROM lncs_front_matter_w.file f
